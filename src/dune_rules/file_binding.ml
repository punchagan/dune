  ...
  | true ->
    (match relative_dst_path_starts_with_parent_error_when with
     | `Deprecation_warning_from_3_11 ->
       Warning_emit.emit warning (Source_dir dir) (fun () ->
         let open Memo.O in
         let+ () = Memo.return () in
...
module Unexpanded = struct
  ...
  let expand t ~dir ~f =
    ...
    let+ dst =
      ...
      | Some dst ->
        ...
        Some (loc, p)
    in
    { src; dst; dune_syntax = t.dune_syntax; dir = Path.Build.drop_build_context_exn dir }
  ;;
  ...
  let decode =
    ...
    in
    let dir = Dune_project.get_exn () >>| Dune_project.root in
    peek_exn
    >>= function
